...
class PlanetaryRulershipCalendarAPI:
    ...
    def generate_calendar(self, year):
        ...
        for month in zodiac_monthly.keys():
            first_day = datetime(year, list(zodiac_monthly.keys()).index(month) + 1, 1)
            last_day = datetime(year, (list(zodiac_monthly.keys()).index(month) + 2) % 12 or 12, 1) - timedelta(days=1)
            days_in_month = (last_day - first_day).days + 1

            calendar_data["months"][month] = {
                "days": days_in_month,
                "zodiac": zodiac_monthly[month][0],
                "rulingPlanet": zodiac_monthly[month][1],
                "days_data": {}
            }
